# AdviceSlipApi2 SDK test runner
...
class AdviceSlipApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AdviceSlipApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AdviceSlipApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AdviceSlipApi2TestRunner.getenv("ADVICE_SLIP_API2_TEST_LIVE")
        override = AdviceSlipApi2TestRunner.getenv("ADVICE_SLIP_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AdviceSlipApi2TestRunner.getenv(key)
        ...
        explain = AdviceSlipApi2TestRunner.getenv("ADVICE_SLIP_API2_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AdviceSlipApi2TestRunner._test_control is not None:
            return AdviceSlipApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AdviceSlipApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AdviceSlipApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AdviceSlipApi2TestRunner.load_test_control()
...
def load_env_local():
    AdviceSlipApi2TestRunner.load_env_local()


def env_override(m):
    return AdviceSlipApi2TestRunner.env_override(m)


def entity_data(v):
    return AdviceSlipApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AdviceSlipApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AdviceSlipApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AdviceSlipApi2TestRunner.load_test_control()


def live_client_options():
    return AdviceSlipApi2TestRunner.live_client_options()


def live_delay_ms():
    return AdviceSlipApi2TestRunner.live_delay_ms()
