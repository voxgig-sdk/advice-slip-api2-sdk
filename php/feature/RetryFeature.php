...
// AdviceSlipApi2 SDK retry feature
...
class AdviceSlipApi2RetryFeature extends AdviceSlipApi2BaseFeature
{
    ...
    public function init(AdviceSlipApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AdviceSlipApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AdviceSlipApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AdviceSlipApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
