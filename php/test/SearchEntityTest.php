...
class SearchEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ADVICE_SLIP_API2_TEST_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function search_basic_setup($extra)
{
    ...
    $client = AdviceSlipApi2SDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("ADVICE_SLIP_API2_TEST_SEARCH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["ADVICE_SLIP_API2_TEST_SEARCH_ENTID"]);
