...
// AdviceSlipApi2 SDK feature test
//
...
class FtCtrl extends AdviceSlipApi2Control
{
...
class FtClient
{
    ...
    private AdviceSlipApi2Utility $utility;

    public function __construct(AdviceSlipApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): AdviceSlipApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AdviceSlipApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AdviceSlipApi2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AdviceSlipApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AdviceSlipApi2Config::make_feature($name);
    ...
    private static function buildUrl(AdviceSlipApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AdviceSlipApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AdviceSlipApi2Response($fetched);
        }
    ...
    private function fail(AdviceSlipApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AdviceSlipApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
