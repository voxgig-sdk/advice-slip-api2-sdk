...
//     (\`new AdviceSlipApi2SDK(...)\` / \`AdviceSlipApi2SDK.test(...)\` become
//     \`AdviceSlipApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AdviceSlipApi2SDK'
...
// rewrite real client construction (\`new AdviceSlipApi2SDK(...)\` and any
// \`AdviceSlipApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AdviceSlipApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AdviceSlipApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
