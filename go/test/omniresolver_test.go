...
import (
	...
	omni "github.com/voxgig-sdk/advice-slip-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AdviceSlipApi2Error); is {
				out := map[string]any{
					"name":    "AdviceSlipApi2Error",
...
func omniCtx(arg any, client *sdk.AdviceSlipApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AdviceSlipApi2SDK); is {
			client = live
