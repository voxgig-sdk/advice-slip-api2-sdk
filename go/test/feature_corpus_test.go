...
//
// NewAdviceSlipApi2SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.AdviceSlipApi2SDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewAdviceSlipApi2SDK(opts)
}
...
func fcCandidates(client *sdk.AdviceSlipApi2SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.AdviceSlipApi2SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.AdviceSlipApi2SDK, name string) any {
