// Command AdviceSlipApi2-mcp is an MCP server exposing the AdviceSlipApi2
// SDK as MCP tools.
//
// Two tools are registered:
//
//	AdviceSlipApi2_list  — list records of an entity
//	AdviceSlipApi2_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: ADVICE_SLIP_API2_APIKEY carries the API key and
	// ADVICE_SLIP_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ADVICE_SLIP_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ADVICE_SLIP_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAdviceSlipApi2SDK(opts)
