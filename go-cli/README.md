# advice-slip-api2-cli

boru-driven command-line client **and** interactive REPL for the AdviceSlipApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/advice-slip-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./advice-slip-api2-cli --help

# 3. Provide credentials once, via the environment
export ADVICE_SLIP_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./advice-slip-api2-cli load 1 advice            # {id:1} shorthand
./advice-slip-api2-cli load '{id:1}' advice       # explicit match map

# 5. Override the API base URL for a single call
ADVICE_SLIP_API2_BASE=https://api.example.com ./advice-slip-api2-cli load 1 advice

# 6. No arguments -> interactive REPL
./advice-slip-api2-cli
advice-slip-api2> load 1 advice
advice-slip-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/advice-slip-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/advice-slip-api2-cli load 1 advice
   ```
...
```sh
./advice-slip-api2-cli load 1 advice          # scalar shorthand for {id:1}
./advice-slip-api2-cli load '{id:1}' advice     # explicit match map
```
...
./advice-slip-api2-cli load 1 advice
```
...
Run with no arguments to open a REPL (prompt `advice-slip-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./advice-slip-api2-cli
advice-slip-api2> load 1 advice
advice-slip-api2> /help
advice-slip-api2> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/advice-slip-api2-cli`. |
...
(`./advice-slip-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
