...
module AdviceSlipApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AdviceSlipApi2Response.new({})
    ...
    if fetch_err
      ...
      response = AdviceSlipApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AdviceSlipApi2Response.new(fetched)
    else
