...
require_relative 'AdviceSlipApi2_types'
...
class AdviceSlipApi2SDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AdviceSlipApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AdviceSlipApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AdviceSlipApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AdviceSlipApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AdviceSlipApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AdviceSlipApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AdviceSlipApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AdviceSlipApi2Error.new(
        "#{op}_allow",
        "AdviceSlipApi2SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AdviceSlipApi2Error => err
    ...
    if fetched.is_a?(Hash)
      status = AdviceSlipApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AdviceSlipApi2Error.new(
        "graphql_error", "AdviceSlipApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AdviceSlipApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
