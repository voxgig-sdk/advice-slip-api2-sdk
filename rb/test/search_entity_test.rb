...
class SearchEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ADVICE_SLIP_API2_TEST_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_basic_setup(extra)
  ...
  entid_env_raw = ENV["ADVICE_SLIP_API2_TEST_SEARCH_ENTID"]
