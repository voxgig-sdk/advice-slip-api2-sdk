# AdviceSlipApi2 SDK Advice entity
...
class AdviceEntity
  ...
  def data_set(args)
    if args
      @_data = AdviceSlipApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = AdviceSlipApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Advice, Hash] the loaded Advice; raises AdviceSlipApi2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AdviceSlipApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
